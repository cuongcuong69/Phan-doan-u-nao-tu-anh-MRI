...
- Giảm LR: nếu val_dice_struct (avg WT/TC/ET) không cải thiện sau `patience` lần eval
  (và epoch >= min_epoch) thì nhân LR với `factor` (ví dụ 0.5).
"""
...
# =============================================================================
# Local utilities: AverageMeter, Logger, cal_dice, region dice (torch)
# =============================================================================
...
def set_seed(seed: int = 2025):
    ...
    # Có thể bật deterministic nếu muốn tái lập tuyệt đối:
...
def train_one_epoch(
    ...
    pbar = tqdm(loader, desc=f"[Train-MultiEnc] Epoch {epoch}/{max_epoch}")
...
def validate(
    ...
    pbar = tqdm(loader, desc=f"[Val-MultiEnc]   Epoch {epoch}/{max_epoch}")
    ...
    for batch in pbar:
        ...
        # val loss = CE + Dice cho đơn giản (hoặc w_ce * CE + w_dice * Dice)
        loss = CFG["LOSS"]["w_ce"] * ce + CFG["LOSS"]["w_dice"] * dice_l
        ...
        # ---- multi-class dice per-class via numpy ----
...
def load_checkpoint(
    ...
    # nếu chỉ muốn load model, có thể comment dòng dưới:
...
def main():
    ...
    ckpt_dir = exp_dir / "checkpoints_dice"
    log_dir = exp_dir / "logs_dice"
    ...
    # resume
    start_epoch = 1
    best_val_dice = 0.0   # dùng avg(WT,TC,ET) - chọn best checkpoint
